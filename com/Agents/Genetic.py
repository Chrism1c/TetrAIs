...
class Genetic(BaseGame, ABC):
    ...
    def __init__(self, r_p):
        super().__init__(r_p)
        print('creazione cromosomi')

        c = Chromosome()

    """
    funzione che calcola la mossa da effettuare in base ai valori dei cromosomi, lo score è calcolato usando la funzione
    calTestScore simula il piazzamento di due tetramini (attuale e successivo) e vede in quale posizione ottiene uno 
    score migliore e piazza il tetramino nella posizione ritenuta migliore
    """

    def get_move(self, board, piece, nextPiece, chromosome):
...
class Chromosome:
    ...
    def __init__(self, numChrom):

        print('creazione prima generazione')
        self.gen0 = self.createGen0(numChrom)
