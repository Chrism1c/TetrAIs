...
def show_text_screen(text):
    ### Splash Screen introduttiva che visualizza 'Please wait to continue.' finchè non viene premuto un tasto
...
def draw_status(score, level, best_move):
    ...
    # draw the score text
    score_surf = BASICFONT.render('Score: %s' % score, True, random_color())#TEXTCOLOR)
    score_rect = score_surf.get_rect()
    score_rect.topleft = (WINDOWWIDTH - 150, 20)
    DISPLAYSURF.blit(score_surf, score_rect)

    # draw the level text
    level_surf = BASICFONT.render('Level: %s' % level, True, random_color())#TEXTCOLOR)
    level_rect = level_surf.get_rect()
    level_rect.topleft = (WINDOWWIDTH - 150, 50)
    DISPLAYSURF.blit(level_surf, level_rect)

    # draw the best_move text
    move_surf = BASICFONT.render('Current Move: %s' % best_move, True, random_color())#TEXTCOLOR)
...
def draw_next_piece(piece):
    ...
    # draw the "next" text
    next_surf = BASICFONT.render('Next:', True, random_color())#TEXTCOLOR)
